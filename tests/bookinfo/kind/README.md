...
Build the third kind cluster with MBG, mbgctl, reviews(v3) and rating microservices:
1) Create a Kind cluster with MBG image:
...
First, Initialize the parameters of the test (pods' names and IPs):
    ...
    export MBGCTL3=`kubectl get pods -l app=mbgctl -o custom-columns=:metadata.name`
...
Initialize mbgctl (mbg control):
    ...
    kubectl exec -i $MBGCTL1 -- ./mbgctl start --id "mbgctl1" --ip $MBGCTL1IP --mbgIP $MBG1PODIP:8443 --dataplane mtls --rootCa ./mtls/ca.crt --certificate ./mtls/mbg1.crt --key ./mtls/mbg1.key
...
Initialize mbgctl (mbg control):
    ...
    kubectl exec -i $MBGCTL2 -- ./mbgctl start --id "mbgctl2" --ip $MBGCTL2IP --mbgIP $MBG2PODIP:8443 --dataplane mtls --rootCa ./mtls/ca.crt --certificate ./mtls/mbg2.crt --key ./mtls/mbg2.key
...
Initialize mbgctl (mbg control):
    ...
    kubectl exec -i $MBGCTL3 -- ./mbgctl start --id "mbgctl3" --ip $MBGCTL3IP --mbgIP $MBG3PODIP:8443 --dataplane mtls --rootCa ./mtls/ca.crt --certificate ./mtls/mbg3.crt --key ./mtls/mbg3.key
...
First, send MBG2, MBG3 details information to MBG1 using mbgctl:

    kubectl config use-context kind-mbg-agent1
    kubectl exec -i $MBGCTL1 -- ./mbgctl addPeer --id "MBG2" --ip $MBG2IP --cport 30443
    kubectl exec -i $MBGCTL1 -- ./mbgctl addPeer --id "MBG3" --ip $MBG3IP --cport 30443
...
In this step, we expose the reviews services from MBG2 and MBG3 to MBG1.
    ...
    kubectl config use-context kind-mbg-agent2
    kubectl exec -i $MBGCTL2 -- ./mbgctl expose --serviceId reviews-v2
    kubectl config use-context kind-mbg-agent3
    kubectl exec -i $MBGCTL3 -- ./mbgctl expose --serviceId reviews-v3
