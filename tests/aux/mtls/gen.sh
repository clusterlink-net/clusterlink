...
openssl req -nodes -newkey rsa:2048 -keyout mbg3.key -out mbg3.csr -subj "/CN=IL" -addext "subjectAltName=IP:172.18.0.4,$kindaddress"
# Sign the mbg3 cert
openssl x509 -req -in mbg3.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out mbg3.crt -extfile <(printf "subjectAltName=IP:172.18.0.4,$kindaddress")
