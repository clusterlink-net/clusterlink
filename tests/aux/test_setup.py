################################################################
#Name: single_test
#Desc: Run iperf3 test between 1 host and 1 target via 1 sn.
#      this test do the following:
#      -Create host, sn and target k8s clusters     
#      -Deploy iperf3 pods on each target and host cluster
#      -Deploy forwarding and hasn pods on sn cluster
#      -Run 3 types of iperf3 test between host to target:
#           - direct connection
#           - using hasn pod(tcp splitting) 
...
def clusterSetup(host, sn,target):
    #creating source, target, sn clusters
    create_k8s_cluster(cluster=host,run_in_bg=True)
    create_k8s_cluster(cluster=sn,run_in_bg=True)
    ...
    checkClusterIsReady(sn)
    deployServiceNode(sn)
...
def hostServiceSetup(host, sn, target ,service):
    checkClusterIsReady(host)
    setupClientService(sn=sn, target=target, service=service)

def testService(service,time=10):
    print(f"Start test for service {service}")
    iperf3Test(target_ip="client-sn-service", target_port="5001",time=time)

def testServiceDB(service,target_ip, target_port, resFolder,time=10):
    print(f"Start test for service {service}")
    if service != "Direct":
        target_ip="client-sn-service"
        target_port="5001"
...
def getFolderRes(host,target,sn,resBase=""):
    time_s = getTime()
    #get folder path
    resDir= f'{resBase}//host-{host.zone}_{host.platform}/target-{target.zone}_{target.platform}/sn-{sn.zone}_{sn.platform}/time-{time_s}/'
...
if __name__ == "__main__":
    ...
    parser.add_argument("-sn_zone"    , "--sn_zone"     , default = "us-east1"   , help="describe sn zone")
    parser.add_argument("-sn_platform", "--sn_platform" , default = "gcp"        , help="describe sn k8s cloud platform")
    ...
    sn_zone         = args.sn_zone 
    sn_platform     = args.sn_platform
    sn_name         = "sn-cluster"
