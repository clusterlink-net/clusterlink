...
func (g *Gwctl) AddService(id, target, port, description string) error {
	...
	j, err := json.Marshal(apiObject.ServiceRequest{Id: id, Ip: target, Port: port, Description: description})
	if err != nil {
		return err
	}
	_, err = httputils.HttpPost(address, j, g.GetHttpClient())
...
func (g *Gwctl) ExposeService(svcId, peer string) error {
	...
	j, err := json.Marshal(apiObject.ExposeRequest{Id: svcId, Ip: "", MbgID: peer})
	if err != nil {
		return err
	}
	//send expose
...
func (g *Gwctl) GetRemoteService(id string) ([]apiObject.ServiceRequest, error) {
	d, _ := config.GetConfig(g.Id)
	mbgIP := d.GetMbgIP()
	...
	resp, err := httputils.HttpGet(address, g.GetHttpClient())
	if err != nil {
		return []apiObject.ServiceRequest{}, err
	}
	var sArr []apiObject.ServiceRequest
	if err := json.Unmarshal(resp, &sArr); err != nil {
		return []apiObject.ServiceRequest{}, err
	}
	for i, s := range sArr {
...
func (g *Gwctl) GetRemoteServices() (map[string][]apiObject.ServiceRequest, error) {
	d, _ := config.GetConfig(g.Id)
	mbgIP := d.GetMbgIP()
...
func (g *Gwctl) RemovePeer(id string) error {
	...
	mbgIP := d.GetMbgIP()
	address := g.GetAddrStart(d.GetDataplane()) + mbgIP + "/peer/" + id
	j, err := json.Marshal(apiObject.PeerRemoveRequest{Id: id, Propagate: true})
	if err != nil {
		return err
	}
...
func (g *Gwctl) RemoveLocalServiceFromPeer(serviceId, peer string) {
	...
	j, err := json.Marshal(apiObject.ServiceDeleteRequest{Id: serviceId, Peer: peer})
	if err != nil {
...
func (g *Gwctl) RemoveRemoteService(serviceId, serviceMbg string) {
	...
	j, err := json.Marshal(apiObject.ServiceRequest{Id: serviceId, MbgID: serviceMbg})
	if err != nil {
		fmt.Printf("Unable to marshal json: %v", err)
	}
...
func (g *Gwctl) CreateServiceEndpoint(serviceId string, port int, name, namespace, mbgAppName string) error {
	...
	j, err := json.Marshal(apiObject.BindingRequest{Id: serviceId, Port: port, Name: name, Namespace: namespace, MbgApp: mbgAppName})
	if err != nil {
		return err
	}
